...
class Ingredient(models.Model):
    ...
    measurement_unit = models.CharField(
        max_length=50, null=False, verbose_name="Единица измерения"
    )
...
class Recipe(models.Model):
    ...
    ingredients = models.ManyToManyField(
        Ingredient, related_name="ingredients", through="RecipeIngredient"
    )
...
class RecipeTag(models.Model):
    recipe = models.ForeignKey(
        Recipe, on_delete=models.CASCADE, related_name="tag_recipe"
    )
...
class ShoppingCart(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["user", "recipe"],
                name="unique user recipe shoppingcart"
            )
        ]
